...
2. **Escalation router** — GenLayer → Kleros. Activating the router registers and activates the GenLayer child; when that child resolves, an escalation window starts (window length from the EAS adjudication definition). Unescalated → the router adopts the child resolution, emits `Adjudicated`. Escalated → the router registers and activates the Kleros child and stays `Active`. Escalation is requested through the router's own `payable` entry point (e.g., `escalate(adjudicationId)`); how escalation is funded is future work.
3. **X-of-Y panel** — registers and activates child adjudications on Y Adjudicators at once (2-of-3 across GenLayer, Kleros, UMA; 11 off-chain agents by majority); resolves when X children agree, per an aggregation rule referenced in the adjudication definition.
4. **No-adjudication passthrough** — the uncontested path: if nobody contests within a window, the passthrough itself activates and resolves the case without invoking any child (both transitions in one transaction — the core lifecycle permits no skipped states, so `Active` is passed through, each event emitted); if contested, it registers and activates a child on the configured Adjudicator. Lets integrators wire the standard in everywhere while paying for adjudication only on actual contests.
...
4. **Echo adjudicator (testing)** — resolves on activation with the resolution it was given at registration (passed via `data` or the adjudication definition), honoring the full `Registered → Active → Resolved` lifecycle. No decision logic at all: a test double for exercising Adjudicables, composites, and integrations end-to-end without any real system behind it.
