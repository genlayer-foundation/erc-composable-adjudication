...
- **An adjudication explorer** (speculative, in the spirit of ERC-8004's ecosystem): an indexer and UI that, given any Adjudicator deployment, shows all its registrations, activations, evidence, and resolutions, and walks composite trees via the introspection interface. The event design already supports this (indexed, uniform streams); the work is ecosystem tooling more than standardization, though a note on what explorers index could be standardized.
- **A main deployment registry** (speculative): an on-chain registry listing Adjudicator deployments (leaves, composites, factories) with their configuration attestations, unifying discovery across factories and implementations, in the spirit of ERC-8004's registries. Per-factory discovery already exists via `CompositeDeployed` and enumeration; a cross-factory registry may not be needed, and is recorded here only as a possibility.
