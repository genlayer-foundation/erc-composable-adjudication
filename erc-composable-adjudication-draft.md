...
2. **`activate(uint256 adjudicationId, bytes32[] evidenceUIDs, bytes data) payable`**
   ...
   - **Authorization is governed by the adjudication definition.** Where the definition names the case's parties, the Adjudicator SHOULD restrict `activate` to them (each party unilaterally — a contested counterparty's cooperation cannot be required). Open activation by anyone is legitimate where the definition opts into it (e.g., oracle-style questions). Absent any specification in the definition, implementations SHOULD restrict activation to the registrant or the parties named in the definition.
...
- A definition MAY be an off-chain attestation (registration then needs only its UID), but its full content MUST be available to the Adjudicator no later than activation, and definitions SHOULD embed content or content hashes rather than mutable references (such as URLs), so that substitution or withholding of the baseline is detectable. Parties SHOULD each retain a copy of any off-chain definition attestation.
